...
public class HeroController : MonoBehaviour {
        ...
        for (int i = 0; i < others.Length; i++)
        {
            ...
            {//检测刚体
                ...
                if (Mathf.Abs(lookAtRot.eulerAngles.y - direction.y) <= 30.0f)
                {
                    ...
                    hero.GetComponent<HeroController>().HPControl(attack_power);
                    //显示伤害
                    string heroName = hero.name;
                    GameObject hitedTarget = GameObject.Find(heroName + "hited");
                    hitedTarget.GetComponent<showHurt>().showHurtNum(attack_power);
                }
            }
        }
    }
    ...
    public void IsGameOver()
    {
        if (life <= 0)
        {
            ...
            if(name== "Base1(Clone)"||name== "Base2(Clone)")
            ToGameOver(1);
    ...
    {
        GameController.whoWin = winOrLost;
        GameController.game_process = GameController.Game_process.Game_over;
    }
