...
public class bulletManager : MonoBehaviour {
    ...
    {
        int  networkPlayer = int.Parse(Network.player + "");
        //产生爆炸效果
        if (hitObject.name == "Plane")
        {
            GameObject effect = Network.Instantiate(effectPrefab, transform.position, Quaternion.identity, 0) as GameObject;
            ...
            GameManager.HeroTurnToFront();
            if (networkPlayer == GameManager.whosRound) GameManager.game_process = GameManager.Game_process.Choose_hero;
        }
    }
